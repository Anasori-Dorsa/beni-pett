...
export const Route = createFileRoute("/order-success")({
  validateSearch: (s) => z.object({ id: z.string().optional(), paid: z.string().optional() }).parse(s),
      ...
      <div className="max-w-lg mx-auto text-center bg-background rounded-3xl border border-border/60 p-10 shadow-[var(--shadow-soft)]">
        {failed ? (
          <>
            ...
            <h1 className="mt-6 font-display text-3xl text-espresso">{t("payment_failed")}</h1>
          </>
        ) : (
          <>
            <CheckCircle2 className="mx-auto h-16 w-16 text-emerald-600" />
            <h1 className="mt-6 font-display text-3xl text-espresso">{t("payment_success")}</h1>
        ...
        <div className="mt-8 flex gap-3 justify-center">
          ...
          {failed && <Link to="/checkout" className="btn-ghost">{t("cart_checkout")}</Link>}
        </div>
      </div>
    </main>
  );
